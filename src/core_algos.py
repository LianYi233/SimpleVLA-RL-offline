...
def compute_mix_grpo_outcome_advantage(token_level_rewards: torch.Tensor,
                                   eos_mask: torch.Tensor,
                                   index: torch.Tensor,
    ...
    with torch.no_grad():
        bsz = scores.shape[0]
        for i in range(bsz):
            if not demo_mask[i].any():     # only on-policy sample
...
def compute_policy_loss(old_log_prob, log_prob, advantages, eos_mask, clip_ratio_high, clip_ratio_low, clip_ratio_c=3.0):
    ...
    cliped_pg_losses1 = torch.max(pg_losses1, pg_losses2)

    pg_clipfrac = verl_F.masked_mean(torch.gt(pg_losses2, pg_losses1).float(), eos_mask)

    pg_losses3 = -advantages * clip_ratio_c
    cliped_pg_losses2 = torch.min(pg_losses3, cliped_pg_losses1)

    pg_losses = torch.where(advantages < 0, cliped_pg_losses2, cliped_pg_losses1)

    # off-policy loss for demonstration data
    # off_ratio = torch.exp(log_prob)
    # off_ratio = off_ratio / (off_ratio + 0.1)
    # off_pg_losses1 = - advantages * off_ratio

    # demo_mask = demo_mask.float()
    # mix_pg_losses = off_pg_losses1 * demo_mask + pg_losses * (1 - demo_mask)
