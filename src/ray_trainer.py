...
def compute_advantage(data: DataProto, gamma, lam, adv_estimator, config):
    ...
    if adv_estimator == 'rloo':
        ...
        steps_expanded = steps.unsqueeze(0).expand(data.batch['responses'].size(0), -1)
        response_mask = steps_expanded < finish_step.unsqueeze(1)  # (batch_size, traj_len)
        advantages, returns = core_algos.compute_mix_grpo_outcome_advantage(token_level_rewards=token_level_rewards,
                                                                        eos_mask=response_mask,
                                                                        index=index,
                                                                        demo_mask=data.batch['demo_mask'])
        data.batch['advantages'] = advantages
        data.batch['returns'] = returns
...
class RayTrainer(object):
    ...
    def init_workers(self):
        ...
        elif self.config.algorithm.adv_estimator in ['grpo', 'mix_grpo']:
            self.use_critic = False
            ...
            while True:
                ...
                while len(valid_batch) < batch_size * n_samples:
                    ...
                    with Timer(name='gen', text="{name}: {seconds:.1f} seconds") as timer:
                        ...
                        repeat_num = n_samples if not self.config.actor_rollout_ref.rollout.rollout_w_demo else n_samples + 1
                        batch_lst = sum([[newbatch[i:i + 1] for _ in range(repeat_num)] for i in range(len(newbatch))],
                                        [])
                    ...
                    with Timer(name='acc&trunc_filter', text="{name}: {seconds:.1f} seconds") as timer:
                        ...
                        filtered_roll_batch = self.filter(roll_batch.batch['acc'].unsqueeze(1), roll_batch, repeat_num)
                    ...
                    print(
                        f"collected {len(valid_batch)} / {batch_size * repeat_num} rollouts and each prompt has {repeat_num} responses")
                    
                if len(valid_batch) < batch_size * repeat_num:
                    break
                elif len(valid_batch) > batch_size * repeat_num:
                    valid_batch = self.add_to_buffer(valid_batch, batch_size, repeat_num)
