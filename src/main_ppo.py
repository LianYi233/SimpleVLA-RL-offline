...
import torch
from src.ray_trainer import RayTrainer
import warnings
...
def extract_feat(imgs):
    ...
    x = F.adaptive_avg_pool2d(imgs, (14, 14))
...
class RobRewardManager():
    ...
    def _init_feature_extractor(self):
        ...
        self.feature_backbone = torch.nn.Sequential(*list(model.children())[:-1])
        self.feature_backbone.eval()
        self.feature_backbone.to(device)
        
        for param in self.feature_backbone.parameters():
            param.requires_grad = False
        self.img_mean = torch.tensor([0.485, 0.456, 0.406], device=device).view(1, 3, 1, 1)
        self.img_std = torch.tensor([0.229, 0.224, 0.225], device=device).view(1, 3, 1, 1)
        self.device = device
        print(f"[RobRewardManager] Initialized ResNet18 feature extractor on {device}")
    ...
    def __call__(self, data: DataProto):
        ...
        reward_tensor = reward_tensor.reshape((reward_tensor.shape[0],-1)) # batch * 64 * 56
        verifier_reward=torch.zeros_like(reward_tensor, dtype=torch.float32)
        match_reward=torch.zeros_like(reward_tensor, dtype=torch.float32)

        # demo_mask = data.batch['demo_mask'][:, 0, 0]
        ...
        if 'match_rewards' in data.batch:
            reward_scores = data.batch['match_rewards'].cpu().numpy().tolist()

        for i in range(verifier_reward.shape[0]):
            if 'match_rewards' in data.batch:
                match_reward[i,valid_response_length[i]-1] += reward_scores[i]
        ...
        if self.config.verifier.reward_coef != 0:
            ...
            if 'match_rewards' in data.batch:
                reward_tensor += self.config.verifier.reward_coef * match_reward
            else:
...
def main_task(config):
    ...
    if config.actor_rollout_ref.actor.strategy == 'fsdp':
        ...
        from src.fsdp_workers import ActorRolloutRefWorker, CriticWorker, RobActorRolloutRefWorker
        from verl.single_controller.ray import RayWorkerGroup
        ray_worker_group_cls = RayWorkerGroup

    elif config.actor_rollout_ref.actor.strategy == 'megatron':
        assert config.actor_rollout_ref.actor.strategy == config.critic.strategy
        from src.megatron_workers import ActorRolloutRefWorker, CriticWorker, RobActorRolloutRefWorker
    ...
    from src.ray_trainer import ResourcePoolManager, Role
    ...
    if config.reward_model.enable and config.reward_model.rm_coef!=0.:
        if config.reward_model.rm_type == 'normal':
            if config.reward_model.strategy == 'fsdp':
                from src.fsdp_workers import RewardModelWorker
            ...
            from src.fsdp_workers import PRIMERewardModelWorker
