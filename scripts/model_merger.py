...
class BaseModelMerger(ABC):
    ...
    def patch_model_generation_config(self, model):
        ...
        if model.can_generate():
            try:
                model.generation_config = GenerationConfig.from_pretrained(self.hf_model_config_path, trust_remote_code=True)
            except OSError:
                print(
    ...
    def save_hf_model_and_tokenizer(self, state_dict: dict[str, torch.Tensor]):
        ...
        import shutil
        src_path = os.path.join(self.hf_model_config_path, "dataset_statistics.json")
        dst_path = os.path.join(self.config.target_dir, "dataset_statistics.json")
        if os.path.exists(src_path):
            shutil.copy(src_path, dst_path)
            print(f"Copied dataset_statistics.json")
        else:
            print(f"Warning: dataset_statistics.json not found in {self.hf_model_config_path}")
...
class FSDPModelMerger(BaseModelMerger):
    ...
    def _test_state_dict(self, state_dict: dict[str, torch.Tensor]):
        ...
        hf_model = auto_model_class.from_pretrained(self.config.test_hf_dir, torch_dtype=torch.bfloat16, trust_remote_code=True)
