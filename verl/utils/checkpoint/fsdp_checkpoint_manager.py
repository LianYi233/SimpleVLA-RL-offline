...
class FSDPCheckpointManager(BaseCheckpointManager):
    ...
    def save_checkpoint(self, local_path: str, hdfs_path: str = None, global_step: int = 0, max_ckpt_to_keep=None):
        ...
        if self.rank == 0:
            ...
            # dataset_statistics.json for openvla-oft
            import json
            if hasattr(unwrap_model, 'norm_stats'):
                dataset_statistics_path = os.path.join(local_path, "dataset_statistics.json")
                with open(dataset_statistics_path, "w") as f:
                    json.dump(unwrap_model.norm_stats, f, indent=2)
                print(f"[rank-{self.rank}]: Saved dataset_statistics.json to {dataset_statistics_path}")
                print(f"[rank-{self.rank}]: norm_stats keys: {list(unwrap_model.norm_stats.keys())}")
            else:
                print(f"[rank-{self.rank}]: Warning - Model has no norm_stats attribute")
        ...
        if "hf_model" in self.checkpoint_contents:
            ...
            if self.rank == 0:
                ...
                elif "OpenVLA" in model_config.architectures[0]:
                    from transformers import AutoModelForVision2Seq
                    auto_model_cls = AutoModelForVision2Seq
                else:
